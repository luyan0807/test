<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
  <head>
    <title>发布预告</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge"/>
    <meta http-equiv="content-type" content="text/html; charset=utf-8"/>
    <meta name="apple-mobile-web-app-capable" content="yes"/>
    <link href="systemAdmin_1/css/jquery-ui-themes.css" type="text/css" rel="stylesheet"/>
    <link href="systemAdmin_1/css/axure_rp_page.css" type="text/css" rel="stylesheet"/>
    <link href="systemAdmin_1/css/styles.css" type="text/css" rel="stylesheet"/>
    <link href="systemAdmin_1/css/styles1.css" type="text/css" rel="stylesheet"/>
    <script src="systemAdmin_1/js/jquery-1.7.1.min.js"></script>
    <script src="systemAdmin_1/js/jquery-ui-1.8.10.custom.min.js"></script>
    <script src="systemAdmin_1/js/prototypePre.js"></script>
    <script src="systemAdmin_1/js/document.js"></script>
    <script src="systemAdmin_1/js/prototypePost.js"></script>
    <script src="systemAdmin_1/js/data.js"></script>
    <script type="text/javascript">
      $axure.utils.getTransparentGifPath = function() { return 'resources/images/transparent.gif'; };
      $axure.utils.getOtherPath = function() { return 'resources/Other.html'; };
      $axure.utils.getReloadPath = function() { return 'resources/reload.html'; };
    </script>
  </head> 
  <body>
    <div id="base" class="">
  <form enctype="multipart/form-data" action="S_activityServlet?&url=publish" method="post" id="updateImage">
      <!-- Unnamed (矩形) -->
      <div id="u426" class="ax_default label">
        <div id="u426_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u427" class="text">
          <p><span>主题：</span></p>
        </div>
      </div>
      
      <!-- 主题 (文本框) -->
      <div id="u428" class="ax_default text_field" data-label="主题">
        <input id="u428_input" type="text" value="topical" name="topical"/>
      </div>

      <!-- Unnamed (提交按钮) -->
      <div id="u429" class="ax_default html_button">
        <input id="u429_input" type="submit" value="发布"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u430" class="ax_default label">
        <div id="u430_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u431" class="text">
          <p><span>正文：</span></p>
        </div>
      </div>

      <!-- Unnamed (文本框) -->
      <div id="u432" class="ax_default text_field">
      <input id="u432_input" type="text" value="" /> 
      </div>
      
      <!-- Unnamed (矩形) -->
      <div id="u433" class="ax_default label">
        <div id="u433_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u434" class="text">
          <p><span>报名有效时间：</span></p>
        </div>
      </div>

      <!-- 报名有效时间 (文本框) -->
      <div id="u435" class="ax_default text_field" data-label="报名有效时间">
        <input id="u435_input" type="text" value="" name="enrollTime"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u436" class="ax_default label">
        <div id="u436_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u437" class="text">
          <p><span>活动地点：</span></p>
        </div>
      </div>

      <!-- 活动地点 (文本框) -->
      <div id="u438" class="ax_default text_field" data-label="活动地点">
        <input id="u438_input" type="text" value="" name="activityAddr"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u439" class="ax_default label">
        <div id="u439_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u440" class="text">
          <p><span>发布日期：</span></p>
        </div>
      </div>

      <!-- 发布日期 (文本框) -->
      <div id="u441" class="ax_default text_field" data-label="发布日期">
        <input id="u441_input" type="text" value="" name="publishDate"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u442" class="ax_default label">
        <div id="u442_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u443" class="text">
          <p><span>活动时间：</span></p>
        </div>
      </div>

      <!-- 活动时间 (文本框) -->
      <div id="u444" class="ax_default text_field" data-label="活动时间">
        <input id="u444_input" type="text" value="" name="activityDate"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u445" class="ax_default label">
        <div id="u445_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u446" class="text">
          <p><span>主讲人：</span></p>
        </div>
      </div>

      <!-- 主讲人 (文本框) -->
      <div id="u447" class="ax_default text_field" data-label="主讲人">
        <input id="u447_input" type="text" value="" name="speaker"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u448" class="ax_default label">
        <div id="u448_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u449" class="text">
          <p><span>参加对象：</span></p>
        </div>
      </div>

      <!-- 参加对象 (文本框) -->
      <div id="u450" class="ax_default text_field" data-label="参加对象">
        <input id="u450_input" type="text" value="" name="actor" />
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u451" class="ax_default label">
        <div id="u451_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u452" class="text">
          <p><span>活动人数：</span></p>
        </div>
      </div>

      <!-- 活动人数 (文本框) -->
      <div id="u453" class="ax_default text_field" data-label="活动人数">
        <input id="u453_input" type="text" value="" name="activityCum"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u454" class="ax_default label">
        <div id="u454_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u455" class="text">
          <p><span>活动内容：</span></p>
        </div>
      </div>

      <!-- 活动内容 (文本框) -->
      <div id="u456" class="ax_default text_field" data-label="活动内容">
        <input id="u456_input" type="text" value="" name="actContent"/>
      </div>

      <!-- 上传图片 (文本框) -->
      <div id="u457" class="ax_default text_field" data-label="上传图片">
        <input id="u457_input" type="file" value="" name="image"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u458" class="ax_default label">
        <div id="u458_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u459" class="text">
          <p><span>上传图片：</span></p>
        </div>
      </div>

      <!-- 温馨提示 (文本框) -->
      <div id="u460" class="ax_default text_field" data-label="温馨提示">
        <input id="u460_input" type="text" value="" name="promptWarmth"/>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u461" class="ax_default label">
        <div id="u461_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u462" class="text">
          <p><span>温馨提示：</span></p>
        </div>
      </div>
      
    </form>  
      

      <!-- Unnamed (矩形) -->
      <div id="u463" class="ax_default box_1">
        <div id="u463_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u464" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u465" class="ax_default image">
        <img id="u465_img" class="img " src="systemAdmin_1/image/u46.png"/>
        <!-- Unnamed () -->
        <div id="u466" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u467" class="ax_default label">
        <div id="u467_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u468" class="text">
          <p><span>借阅排行</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u469" class="ax_default image">
        <img id="u469_img" class="img " src="systemAdmin_1/image/u50.png"/>
        <!-- Unnamed () -->
        <div id="u470" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u471" class="ax_default label">
        <div id="u471_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u472" class="text">
          <p><span>额度标准</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u473" class="ax_default image">
        <img id="u473_img" class="img " src="systemAdmin_1/image/u54.png"/>
        <!-- Unnamed () -->
        <div id="u474" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u475" class="ax_default label">
        <div id="u475_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u476" class="text">
          <p><span>三只公告</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u477" class="ax_default image">
        <img id="u477_img" class="img " src="systemAdmin_1/image/u58.png"/>
        <!-- Unnamed () -->
        <div id="u478" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u479" class="ax_default label">
        <div id="u479_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u480" class="text">
          <p><span>数据备份</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u481" class="ax_default image">
        <img id="u481_img" class="img " src="systemAdmin_1/image/u62.png"/>
        <!-- Unnamed () -->
        <div id="u482" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u483" class="ax_default label">
        <div id="u483_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u484" class="text">
          <p><span>活动预告</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u485" class="ax_default image">
        <img id="u485_img" class="img " src="systemAdmin_1/image/u66.png"/>
        <!-- Unnamed () -->
        <div id="u486" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u487" class="ax_default label">
        <div id="u487_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u488" class="text">
          <p><span>读者咨询</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u489" class="ax_default image">
        <img id="u489_img" class="img " src="systemAdmin_1/image/u70.png"/>
        <!-- Unnamed () -->
        <div id="u490" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u491" class="ax_default label">
        <div id="u491_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u492" class="text">
          <p><span>超期缴费</span></p>
        </div>
      </div>

      <!-- Unnamed (图片) -->
      <div id="u493" class="ax_default image">
        <img id="u493_img" class="img " src="systemAdmin_1/image/u74.png"/>
        <!-- Unnamed () -->
        <div id="u494" class="text" style="display:none; visibility: hidden">
          <p><span></span></p>
        </div>
      </div>

      <!-- Unnamed (矩形) -->
      <div id="u495" class="ax_default label">
        <div id="u495_div" class=""></div>
        <!-- Unnamed () -->
        <div id="u496" class="text">
          <p><span>个人信息</span></p>
        </div>
      </div>
    </div>
  </body>
</html>
